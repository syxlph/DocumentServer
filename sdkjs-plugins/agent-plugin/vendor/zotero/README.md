# Vendored ONLYOFFICE Zotero plugin

This directory vendors `sdkjs-plugins/content/zotero/` from
`ONLYOFFICE/onlyoffice.github.io` at commit
`b39b42b273f0db5c8593547478b4e37b948211bf`.
...
Those runtime artifacts were previously verified to match the pinned upstream
commit above. Task 4 keeps the vendored Zotero sources essentially unchanged and
adapts the hidden `agent-plugin` through wrapper/bootstrap code in
`sdkjs-plugins/agent-plugin/scripts/`.
